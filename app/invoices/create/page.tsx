...
export default function CreateInvoice() {
  const [form, setForm] = useState<any>({ sales_order_id: undefined, invoice_date: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await apiPost('/invoices', form);
      push('Invoice created', 'success');
      router.push('/invoices');
    } catch (err:any) {
      push('Failed to create invoice: ' + (err?.message ?? String(err)), 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl bg-white p-6 rounded shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <input value={form.invoice_date ?? ''} onChange={e => setForm({...form, invoice_date: e.target.value})} placeholder="Invoice Date" type="date" className="w-full px-3 py-2 border rounded" />
