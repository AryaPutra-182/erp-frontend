'use client';
import { useState } from 'react';
import { apiPost } from '../../lib/api';
import { useToast } from '../../../components/ToastProvider';
import { useRouter } from 'next/navigation';

export default function CreateSalesOrder() {
  const [form, setForm] = useState<any>({ customer_id: undefined, transaction_date: '' });
  const [loading, setLoading] = useState(false);
  const { push } = useToast();
  const router = useRouter();

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.customer_id) return push('Customer ID required', 'error');
    try {
      setLoading(true);
      await apiPost('/sales', form);
      push('Sales order created', 'success');
      router.push('/sales-orders');
    } catch (err:any) {
      push('Failed to create sales order: ' + (err?.message ?? String(err)), 'error');
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="max-w-2xl bg-white p-6 rounded shadow">
      <h2 className="text-xl font-semibold mb-4">Create Sales Order</h2>
      <form onSubmit={handleSubmit} className="space-y-3">
        <input value={form.customer_id ?? ''} onChange={e => setForm({...form, customer_id: Number(e.target.value)})} placeholder="Customer ID" className="w-full px-3 py-2 border rounded" />
        <input value={form.transaction_date ?? ''} onChange={e => setForm({...form, transaction_date: e.target.value})} placeholder="Transaction Date" type="date" className="w-full px-3 py-2 border rounded" />
        <div className="flex gap-2">
          <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded">{loading ? 'Saving...' : 'Save'}</button>
        </div>
      </form>
    </div>
  )
}
